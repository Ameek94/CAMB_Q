...
def main():
    if len(sys.argv)<3:
        print("Usage: python sequential_opt_mpi_opt.py <solver> <maxfun> [nrestart]")
        sys.exit(1)
    ...
    info = yaml_load('./spline_4_vector.yaml')
    ...
    x0 = np.random.uniform(0.0, 1.0, size=(100,len(param_list)))
    inits = []
    vals = []
    for x in x0:
    ...
    for i in range(1, nrestart+1):
        log(f"Restart {i}/{nrestart}, x0 = {initial_x}", start)
        if solver=='scipy':
