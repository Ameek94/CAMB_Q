...
def plot_spline_quintessence(param_dict,
    ...
    *_file : str
    ...
    # baseline LCDM
    ombh2          = 2.222461800e-02 #0.0222246 # 0.0223828 #
    omch2          =  1.187082839e-01 #0.118708 # 0.1201075 #
    omk            = 0.
    hubble         = 6.768350025e+01 #67.6835 # 67.32117 #
    As = 2.096657408e-09
    tau = 5.662117601e-02
    ns = 9.641828167e-01
    pars_lcdm = camb.set_params(ombh2=ombh2, omch2=omch2, H0=hubble, dark_energy_model='fluid',As=As, tau=tau,ns=ns)
    ...
    # phi_train = param_dict['phi_train']
    ...
    phi_train = param_dict['phi_train']
    V_train = param_dict['V_train']
    phis = np.linspace(0.0, np.max(phi_train), 100)
    Vphis_full = np.array(results_spline.get_dark_energy_Vphi(phis))
    Vphis_full /= Vphis_full[0]
    ax[1,0].plot(phis, Vphis_full, color='C1')
    ax[1,0].axvline(np.max(ev_phi[:,0]), ls='--', color='k')
