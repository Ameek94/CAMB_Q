...
def main():
    ...
    info = yaml_load(f'./spline_{nspline}_vector_only_DE.yaml')
    # print(info)
    ...
    for i in range(1, nrestart+1):
        ...
        try:
            initial_x = x0[np.random.choice(len(x0))] # best_x.copy()
        except:
    ...
    # save
    fname = f"best_result_{nspline}_{solver}_{maxfun}_planck_lite_onlyDE.txt"
    ...
    plot_spline_quintessence(param_dict=pdict,nspline=nspline,output_prefix=f"SplineQ_planck_lite_onlyDE_{solver}")
